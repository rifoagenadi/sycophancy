...
def get_probe_vectors(chosen_layer, model_id, scale, num_layers, hidden_dim, use_random_direction=None):
    ...
    for layer in target_layers:
        current_probe = torch.load(f'linear_probe/trained_probe_sycophancy/{model_id}/linear_probe_residual_{layer}.pth')['linear.weight'].squeeze()
        ...
        current_std = torch.load(f'linear_probe/trained_probe_sycophancy/{model_id}/std_residual_{layer}.pt')
...
def main():
    ...
    accuracies = pickle.load(open(f'linear_probe/trained_probe_sycophancy/{model_id}/accuracies_dict_residual.pkl', 'rb'))
    config = model.config
    ...
    import pandas as pd
    pd.DataFrame({'question': questions_test, 'correct_answer': correct_answers_test,'initial_answer': initial_answer, 'final_answer': final_answer}).to_csv(f"predictions_sycophancy/{dataset_id}_{model_id}_answers_L{chosen_layer}_{scale}_residual.csv")
